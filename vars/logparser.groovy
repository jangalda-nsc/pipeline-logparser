...
java.util.LinkedHashMap _parseOptions(java.util.LinkedHashMap options = [:])
{
    ...
    // make sure there is no type mismatch when comparing string elements of options.filter
    new_options.filter = new_options.filter.collect{ it != null ? (it in CharSequence ? it.toString() : it) : null }
    return new_options
}
...
String getLogsWithBranchInfo(java.util.LinkedHashMap options = [:], build = currentBuild)
{
    ...
    */
    ...
    tree.values().each {
        ...
        keep."${it.id}" = \
            opt.filter.size() == 0 ||
            (branches.size() == 0 && null in opt.filter) ||
            (branches.size() > 0 && opt.filter.count{ it != null && it in CharSequence && branches[0] ==~ /^${it}$/ } > 0) ||
            (branches.size() > 0 && opt.filter.count{
...
java.util.ArrayList getBranches(java.util.LinkedHashMap options = [:], build = currentBuild)
{
    ...
    tree.values().each {
        ...
        if (keep."${it.id}" && ! (branches in result) ) {
            result += [ branches ]
        }
    }
    return result
